...
def fetch_f1_news(rss_url, days: int, container_attrs=None):
    """
...
def run_f1_news_crawler(
    ...
    Args:
        ...
        container_attrs (dict): Attributes to find the content container in the article.
        output_dir (str): The directory to save the JSON file when debug=True.
    ...
    if debug:
        ...
        fetch_path = os.path.join(output_dir, f"{source_name}_{executed_at}.json")
        with open(fetch_path, "w", encoding="utf-8") as f:
            json.dump(news_data, f, ensure_ascii=False, indent=4)
        logging.info(f"Fetched data saved to {fetch_path}")
    else:
        logging.info("Debug mode is off in Step 1, not saving fetched data.")
